...
ax2.set_ylabel("Speicherplatz (MB)", color=ieee_blue2)
...
for i in range(len(modelle)):
    ...
    ax2.text(x[i] + (breite / 2 + abstand / 2), zeit_pro_epoche[i] + 5, f"{zeit_pro_epoche[i]}MB", ha='center', fontsize=10, color=ieee_blue2)
...
plt.title("Vergleich: Testerfolg vs. Größe des Modells", fontsize=14, fontweight='bold', pad=25)
