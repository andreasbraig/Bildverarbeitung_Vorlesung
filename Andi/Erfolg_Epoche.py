...
zeit_pro_epoche = [227, (4*60+21) , 241, 221, 53]  # Zeit in Sekunden
...
ax2 = ax1.twinx()
balken2 = ax2.bar(x + (breite / 2 + abstand / 2), zeit_pro_epoche, breite, label="Zeit pro Epoche (Sek.)", color=ieee_blue2)
ax2.set_ylabel("Zeit pro Epoche (Sek.)", color=ieee_blue2)
...
for i in range(len(modelle)):
    ...
    ax2.text(x[i] + (breite / 2 + abstand / 2), zeit_pro_epoche[i] + 5, f"{zeit_pro_epoche[i]}s", ha='center', fontsize=10, color=ieee_blue2)
