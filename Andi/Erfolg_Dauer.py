...
ax2.set_ylabel("Zrainingsdauer (Min.)", color=ieee_blue2,fontsize=20)
...
for i in range(len(modelle)):
    ...
    ax2.text(x[i] + (breite / 2 + abstand / 2), zeit_pro_epoche[i] + 5, f"{zeit_pro_epoche[i]}min", ha='center', fontsize=12, color=ieee_blue2)
...
plt.title("Vergleich: Testerfolg vs. Zeit pro Epoche", fontsize=20, fontweight='bold', pad=25)
