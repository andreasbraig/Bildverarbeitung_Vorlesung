...
In der Datei \texttt{data\_generation.py} wird der in Kapitel \ref{sec:dataset_description} beschriebene Datensatz eingelesen und die Bilder mittels der Segmentierungsmasken verarbeitet. Hierbei geschieht die Verarbeitung wie Folgt. Den Bildern wird eine zusätzliche Farbebene (Alpha-Kanal) beigefügt, sowie die abgebildeten Personen freigestellt. Das resultierende Bild wird mittels einer affinen Transformation in eine einheitliche Position und Ausrichtung gebracht. Anschließend werden die Bilder in einem neuen Verzeichnis gespeichert, um sie für das Training des Modells zu verwenden.

wird mittels der Segmentierungsmaske und einer Konturanalyse der Hintergrund entfernt und ein zusätzlicher. Diese Positionen werden genutzt, um die Transformationsmatrix für die affine Transformation zu berechnen. Anschließend werden die Bilder anhand dieser Matrix transformiert, um die Gesichter einheitlich zu positionieren. Zudem erfolgt im gleichen Zuge der Transformation eine Skalierung der Bilder auf eine einheitliche Größe. Die transformierten Bilder werden in einem neuen Verzeichnis gespeichert, um sie für das Training des Modells zu verwenden.
