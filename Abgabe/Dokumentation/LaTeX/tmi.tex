...
\begin{figure}[H]
    ...
    \caption{Trainingsergebnis pro Epoche des letzten Modells.}
    \label{fig:trainlog}
\end{figure}

Die Abbildung \ref{fig:trainlog} zeigt das Trainingsergebnis des letzten Modells. Die Genauigkeit des Modells steigt mit jeder Epoche und nähert sich den $100\%$ asymptotisch an. Zudem sinken die Verluste, bis diese nahezu $0\%$ sind. Dies zeigt, dass das Modell gut trainiert ist und eine hohe Genauigkeit bei der Geschlechterklassifikation erreicht.
Außerdem ist in der Grafik zu sehen, bei welcher Epoche die Data Augmentation angewand wird. Denn hier sind kurzzeitige Verschlechterungen von Genauigkeit und Verlust zu erkennen, die durch die Veränderung des Datensatzes entstehen.

\subsection{Test des Modells}
Das zuvor in Kapitel \ref{sec:training} trainiert und gespeicherte Modell wird neu geladen und für das Testen verwendet. Durch ein erneutes Laden des Modells wird sichergestellt, dass das Modell unabhängig von dem Trainingsprozess getestet werden kann.
Hierbei wird dem Modell batchweise der Testdatensatz übermittelt, welcher sich von dem Traingsdatensatz unterscheidet, übermittelt Die Klassifikationsergebnisse werden mit den tatsächlichen Labels verglichen, um die Genauigkeit des Modells zu bestimmen. Das Ergebnis wird in Form einer Erfolgsquote ausgegeben, die die Genauigkeit des Modells bei der Geschlechterklassifikation angibt. Zudem werden die falsch Klassifizierten Bilder in ein separates Verzeichnis gespeichert, um die Fehleranalyse zu erleichtern.
