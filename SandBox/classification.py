...
def apply_augmentation(image):
    ...
    return torch.cat((augmented_rgb, alpha), dim=0)
...
def train_model(data_dir, device,epochs=5,modelname = "model.state"):
    ...
    trainEpochs = epochs
    if trainEpochs > 0:
        model.trainStart(epochs=trainEpochs, lr=0.001, train_loader=train_dl, device=device, modelname = modelname, opt_func=torch.optim.Adam, patience=5, lr_patience=3, lr_decay_factor=0.5)
...
def test_model(test_data_dir, device, modelname, logfile):
    ...
    model.inferenzSet(test_dataset, device, logfile)
